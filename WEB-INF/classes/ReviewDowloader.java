	...
	public static ArrayList<String> reviewDownload(String reviewLink)
	{
		...
		try
		{
			...
			{	
				...
				ul = doc.select("span.a-size-medium.a-text-beside-button.totalReviewCount");
				totalReview = Integer.parseInt(ul.text().toString());
			...
			for(int i = 2;i<=no_pages+rem;i++)
			{
				...
				return reviews;
			/*
			//to store pages to traverse
			ArrayList<String> traversed = new ArrayList<String>();
			ArrayList<String> toTraverse = new ArrayList<String>();
			...
			{	
					String traversingURL = toTraverse.get(i);
					i++;	
					traversed.add(traversingURL);
					Document doc2 = null;
					while(doc2==null)
  
					doc2 = Jsoup.connect(traversingURL).followRedirects(true).ignoreContentType(true).header("Accept-Language", "pt-BR,pt;q=0.8").header("Accept-Encoding", "gzip,deflate,sdch").timeout(300000).userAgent("Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/"+".0.2228.0 Safari/536.38").get();
					Elements ulNew = doc2.select("ul.a-pagination");
					Elements listNew = ulNew.select("li.page-button");
				
					for(Element newURL:listNew)
					{
						Element  link2 = newURL.select("a").first();
						String url1 =  link2.attr("abs:href");
						if(!traversed.contains(url1)&&(!toTraverse.contains(url1)))
						{
							toTraverse.add(url1);
						}
					}			
			...
			for(String traversingURL: toTraverse)
			{	
